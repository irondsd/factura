...
function databaseTarget(): { host: string; local: boolean } {
  ...
  return {
    host,
    local: new Set(["localhost", "127.0.0.1", "::1", "0.0.0.0", "db"]).has(host),
  };
}
...
function assertExpectedImports(): void {
  ...
  for (const file of fs.readdirSync(dir).filter((name) => name.endsWith(".mdx"))) {
    const imports = declaredImports(fs.readFileSync(path.join(dir, file), "utf8"));
    for (const specifier of imports) {
      if (!EXPECTED_IMPORTS.has(specifier)) {
        throw new Error(`${file}: refusing unexpected import ${JSON.stringify(specifier)}`);
      }
    }
  }
}
...
async function main() {
  ...
  if (!target.local) {
    if (!production || process.env.CMS_IMPORT_PRODUCTION_CONFIRM !== "IMPORT_GUIDES") {
      throw new Error(
  ...
  const stored = (await db.query.cmsPages.findMany({
  ...
  console.log(`Guide import target: ${target.local ? "local" : "production"} (${target.host})`);
  console.log(`${source.length} source guides; ${changed.length} insert/update; ${source.length - changed.length} unchanged.`);
  if (dryRun) return;
  ...
  const imported = (await db.query.cmsPages.findMany({
    where: eq(cmsPages.section, "guias"),
  })).map(rowToDocument);
  const diagnostics = validateContentCollection(imported);
  const problems = [...diagnostics.values()].flat();
  if (problems.length) {
    throw new Error(`Imported guides have ${problems.length} validation diagnostics`);
  }
