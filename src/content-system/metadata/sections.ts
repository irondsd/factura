...
export const sectionMetadataSchema = z.object({
  ...
  previewImage: z.string().regex(/^\/img\/(estadisticas|investigacion)\/previews\/.+\.(jpg|png|webp)$/).optional(),
  sources: z.array(z.object({ label: text, href: z.string().url(), note: text.optional() }).strict()).optional(),
  dataset: z.object({ name: text, description: text, temporalCoverage: text, spatialCoverage: text, variableMeasured: z.array(text) }).strict().optional(),
}).strict();
