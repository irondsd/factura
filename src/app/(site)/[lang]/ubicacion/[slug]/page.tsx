...
export function generateStaticParams() {
  return spanishOnly(async () =>
    (await nonEmptyContentLocations()).map((location) => ({
      slug: location.slug,
    })),
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const location = await locationBySlug((await params).slug);
  return location ? locationMetadata(location) : {};
}
...
export default async function LocationHubPage({ params }: Props) {
  ...
  if (!location) {
    const moved = await locationRedirect(slug);
    if (moved) permanentRedirect(`/ubicacion/${moved.slug}`);
    notFound();
  }
  const groups = await contentInLocation(location.key);
  const ordered = CMS_SECTIONS.map((section) => ({
    section,
    pages: groups[section.id],
      ...
      <main className={SHELL}>
        <Breadcrumbs
          ...
          items={[
            ...
            { name: location.label, href: `/ubicacion/${location.slug}` },
          ]}
        />
        <header className="max-w-[680px] pt-7 pb-2">
          <Eyebrow tone="accent">Ubicación</Eyebrow>
        ...
        <div className="mt-12 mb-16 flex flex-col gap-12">
          {ordered.map(({ section, pages }) => (
            <section key={section.id}>
              <div className="flex items-baseline justify-between gap-4 border-b border-line pb-3">
                <h2 className="m-0 font-display text-[24px] font-semibold tracking-[-0.02em] sm:text-[27px]">
                  {section.label}
                </h2>
                <Eyebrow>
                  {pages.length}{" "}
        ...
        <nav className="mb-16">
          <Link
            href="/ubicacion"
            className="font-mono text-micro tracking-label-wide text-muted uppercase no-underline hover:text-accent"
          >
...
function plural(section: ContentSection): string {
  return section === "guias"
    ? "guías"
    : section === "noticias"
      ? "noticias"
