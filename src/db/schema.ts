...
export const parserTier = pgEnum('parser_tier', ['official', 'verified', 'community'])
...
export const forecastBasis = pgEnum('forecast_basis', ['carry', 'baseline', 'yoy'])

export const forecastConfidence = pgEnum('forecast_confidence', ['low', 'medium', 'high'])
...
export const monthlyReports = pgTable(
  ...
  (t) => [uniqueIndex('monthly_report_property_period_idx').on(t.propertyId, t.period)],
)
...
export const billSubmissions = pgTable(
  'bill_submissions',
  {
    ...
    matchedConfigId: uuid('matched_config_id').references(() => parserConfigs.id, { onDelete: 'set null' }),
    ...
    matchedVersionId: uuid('matched_version_id').references(() => parserVersions.id, { onDelete: 'set null' }),
  ...
  (t) => [
    ...
    index('bill_submission_sweep_idx')
      .on(t.createdAt)
      .where(sql`${t.keepFile} = false and ${t.claimedByUserId} is null and ${t.storageKey} is not null`),
