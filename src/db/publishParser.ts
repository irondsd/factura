...
import { parserConfigs } from "./schema";
import { ensureSystemUser, publishPackage } from "../server/registry";
 ...
 *
 * Usage: npx dotenv -e .env.local tsx src/db/publishParser.ts <slug>
 * Idempotent: re-publishing an already-published version is a no-op.
 */
async function main() {
  const slug = process.argv[2];
  if (!slug) {
    console.error("Usage: tsx src/db/publishParser.ts <slug>");
    process.exit(1);
  }
