...
import { canAuthor, canManageTokens, canPublish, resolveCmsAccess } from './policy'
...
describe('resolveCmsAccess', () => {
  ...
  it('admits an editor with their role, name and email', () => {
    expect(resolveCmsAccess({ id: 'u2', email: 'e@example.com', name: 'Eve' }, { role: 'editor' })).toEqual({
      kind: 'member',
      actor: editor,
    })
  })
  ...
  it('admits an admin', () => {
    expect(resolveCmsAccess({ id: 'u1', email: 'a@example.com', name: 'Ada' }, { role: 'admin' })).toEqual({
      kind: 'member',
      actor: admin,
    })
  })
