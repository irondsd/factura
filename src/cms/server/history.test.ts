...
const lockOf = async (fake: ReturnType<typeof createFakeCms>, id: string): Promise<number> =>
...
describe('recording page history', () => {
  ...
  it('tells restoring, discarding and promoting apart from an ordinary save', async () => {
    ...
    const publication = versions.versions.find((version) => version.kind === 'published')!
    ...
    expect(actionsOf(fake)).toEqual(['created', 'preview_promoted', 'status', 'restored', 'discarded'])
  })
