...
import type { ContentDocument, ContentStatus, ValidationResult } from '@/content-system/types'
...
if (!hasTestDatabase()) {
  ...
  describe('CMS content service against PostgreSQL', () => {
    ...
    const service = new CmsContentService(permissive, store, revisions, history, undefined, noInvalidation)
    ...
    const lockOf = async (id: string): Promise<number> => (await service.getState(actor, id)).lockVersion

    /** Save the working copy and answer with the document, which is what most
     * of the assertions below are about. */
    const save = async (id: string, patch: Parameters<typeof service.update>[1]['patch']) =>
      (
      ...
      await db
        ...
        .where(
          inArray(
            ...
            db.select({ id: cmsSchema.cmsPages.id }).from(cmsSchema.cmsPages).where(mine),
          ),
        )
    ...
    describe('lifecycle visibility', () => {
      it('hides a draft from every public read', async () => {
        ...
        expect((await repository.listPublished('guias')).map((s) => s.slug)).not.toContain(page.slug)
        expect((await repository.listPubliclyRenderable('guias')).map((s) => s.slug)).not.toContain(page.slug)
      })
      ...
      it('renders a preview at its URL but keeps it out of listings', async () => {
        ...
        expect(await repository.getByPath('guias', [previewed.slug])).not.toBeNull()
        expect((await repository.listPublished('guias')).map((s) => s.slug)).not.toContain(previewed.slug)
        // The one list it does belong to: callers that need to know a path
        // resolves, such as generateStaticParams.
        expect((await repository.listPubliclyRenderable('guias')).map((s) => s.slug)).toContain(previewed.slug)
      })
      ...
      it('renders and lists a published page', async () => {
        ...
        expect((await repository.listPublished('guias')).map((s) => s.slug)).toContain(live.slug)
      })
      ...
      it('refuses a duplicate slug', async () => {
        ...
        await expect(service.create(actor, draftInput('dupe'))).rejects.toThrow(CmsSlugTakenError)
      })
    })
    ...
    describe('optimistic concurrency', () => {
      ...
      it('reports the version actually in the database', async () => {
        const page = await service.create(actor, draftInput('conflict-version'))
        await service.update(actor, {
          id: page.id,
          expectedLockVersion: page.lockVersion,
          patch: { title: 'Uno' },
        ...
        expect((error as CmsConflictError).expectedLockVersion).toBe(page.lockVersion)
        expect((error as CmsConflictError).actualLockVersion).toBe(page.lockVersion + 1)
      })
    ...
    describe('timestamps', () => {
      it('moves the editorial timestamp on a content edit', async () => {
        ...
        expect(Date.parse(edited.contentUpdatedAt)).toBeGreaterThanOrEqual(Date.parse(page.contentUpdatedAt))
      })
      ...
      it('levels the editorial timestamp on a first publication', async () => {
        ...
        expect(Date.parse(live.contentUpdatedAt)).toBeGreaterThanOrEqual(Date.parse(live.publishedAt as string))
      })
    ...
    describe('validation gate', () => {
      ...
      const failsPublishOnly = ({ level }: { level: string }): ValidationResult =>
        level === 'publish'
          ? {
              ok: false,
              diagnostics: [{ code: 'test.rejected', severity: 'error', message: 'nope' }],
            }
          : { ok: true, diagnostics: [] }
      ...
      it('refuses to publish content that does not validate', async () => {
        ...
        const strict = new CmsContentService(failsPublishOnly, store, revisions, history, undefined, noInvalidation)
      ...
      it('blocks a save that fails even at draft level', async () => {
        ...
        const always = (): ValidationResult => ({
          ok: false,
          diagnostics: [{ code: 'test.forbidden', severity: 'error', message: 'no JS' }],
        })
        const strict = new CmsContentService(always, store, revisions, history, undefined, noInvalidation)
    ...
    describe('hierarchy', () => {
      ...
      it('resolves a nested public path', async () => {
        ...
        const found = await repository.getByPath('guias', child.slug.split('/'))
    ...
    describe('preview is absent from every discovery surface', () => {
      ...
      it('is renderable at its URL but in no published listing', async () => {
        ...
        expect(await repository.getByPath('guias', page.slug.split('/'))).not.toBeNull()
        ...
        expect(relatedDocuments(page, published).map((s) => s.slug)).not.toContain(page.slug)
      })
      ...
      it('appears in listPubliclyRenderable, which is not a listing', async () => {
        ...
        expect((await repository.listPubliclyRenderable('guias')).map((s) => s.slug)).toContain(page.slug)
      })

      it('keeps a draft out of even that', async () => {
        const draft = await service.create(actor, draftInput('draft-hidden'))
        expect((await repository.listPubliclyRenderable('guias')).map((s) => s.slug)).not.toContain(draft.slug)
        expect(await repository.getByPath('guias', draft.slug.split('/'))).toBeNull()
      })
    ...
    describe('database adapter', () => {
      it('returns every state, unlike the public repository', async () => {
        ...
        expect((await repository.listPublished('guias')).map((s) => s.slug)).not.toContain(draft.slug)
      })
    })
    ...
    describe('stored rows', () => {
      ...
      it('stores the section/slug pair uniquely', async () => {
        ...
        const rows = await db.select().from(cmsSchema.cmsPages).where(eq(cmsSchema.cmsPages.slug, page.slug))
    ...
    describe('deletion', () => {
      it('removes a draft from the database for good', async () => {
        ...
        await expect(service.get(actor, page.id)).rejects.toBeInstanceOf(CmsNotFoundError)
      })
    ...
    describe('page history', () => {
      it('records the whole life of a page, newest first', async () => {
        ...
        const entries = await loadPageHistory((await store.findById(page.id))!, history)

        expect(entries.map((entry) => [entry.action, entry.did, entry.inferred])).toEqual([
    ...
    describe('revisions in the database', () => {
      ...
      const published = (document: ContentDocument | null) => document && { ...document, lockVersion: 0 }

      const revisionsOf = (pageId: string) =>
        db.select().from(cmsSchema.cmsPageRevisions).where(eq(cmsSchema.cmsPageRevisions.pageId, pageId))
      ...
      it('keeps the published revision byte-for-byte while the draft changes', async () => {
        ...
        await publish(page.id)
        const before = published(await repository.getByPath('guias', [page.slug]))

        await save(page.id, { body: 'Un borrador a medio escribir.\n' })
        await save(page.id, { title: 'Título provisional' })

        expect(published(await repository.getByPath('guias', [page.slug]))).toEqual(before)
      })
      ...
      it('refuses to delete a revision the page still points at', async () => {
        ...
        await expect(
          db.delete(cmsSchema.cmsPageRevisions).where(eq(cmsSchema.cmsPageRevisions.id, publication.id)),
        ).rejects.toThrow()
      })
      ...
      it('leaves one working copy and one checkpoint after a run of saves', async () => {
        ...
        const kinds = (await revisionsOf(page.id)).map((revision) => revision.kind).sort()
      ...
      it('serves the promoted snapshot, not the latest save, while in preview', async () => {
        ...
        const promoted = published(await repository.getByPath('guias', [page.slug]))

        await save(page.id, { body: 'Algo que nadie debería ver todavía.\n' })

        expect(published(await repository.getByPath('guias', [page.slug]))).toEqual(promoted)
        expect((await service.getState(actor, page.id)).previewIsStale).toBe(true)
      })
      ...
      it('restores an old publication into the working copy without moving the live page', async () => {
        ...
        await publish(page.id)
        const liveBefore = published(await repository.getByPath('guias', [page.slug]))
        ...
        expect(published(await repository.getByPath('guias', [page.slug]))).toEqual(liveBefore)
    ...
    describe('metadata is checked before it is written', () => {
      ...
      it('writes no row when it refuses a create', async () => {
        await service.create(actor, { ...draftInput('bad-create-row'), metadata: bad }).catch(() => {})
        const rows = await db
          .select()
      ...
      it('names the offending field, so the editor can point at it', async () => {
        ...
        expect((error as CmsValidationError).diagnostics.map((d) => d.field)).toContain('previewMediaId')
      })
      ...
      it('refuses to serve a damaged row publicly', async () => {
        ...
        await expect(repository.getByPath('guias', [page.slug])).rejects.toThrow(/invalid metadata/)
      })
    })
  })
}
