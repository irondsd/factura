...
import { CmsConflictError, CmsMediaInUseError, CmsValidationError } from '../../server/errors'
...
if (!hasTestDatabase() || !isMediaStorageConfigured()) {
  ...
  describe('CMS media library', () => {
    ...
    async function upload(filename: string, bytes?: Buffer, collectionId?: string | null) {
    ...
    async function cleanup() {
      ...
      if (ids.length) {
        await db.delete(schema.cmsMediaUsage).where(inArray(schema.cmsMediaUsage.mediaId, ids))
        for (const id of ids) {
        ...
        await db.delete(schema.cmsMedia).where(inArray(schema.cmsMedia.id, ids))
      }
      created.clear()
      await db.delete(schema.cmsMediaCollections).where(like(schema.cmsMediaCollections.name, `${TEST_PREFIX}%`))
      ...
      await db
        ...
        .where(
          inArray(
            ...
            db.select({ id: schema.cmsPages.id }).from(schema.cmsPages).where(mine),
          ),
        )
    ...
    describe('upload', () => {
      it('reserves a row before the presigned URL exists, so no object can be unknown', async () => {
        ...
        expect((await store.objectKeysOf(reservation.mediaId))?.stagingKey).toBeTruthy()
      })
      ...
      it('stores a master, records its real dimensions, and clears the staging key', async () => {
        ...
        expect(keys?.objectKey).toMatch(new RegExp(`^${MEDIA_PREFIX}/${asset.id}/[0-9a-f]{16}\\.png$`))
      })
      ...
      it('leaves no staging object behind once finalization succeeds', async () => {
        ...
        expect(keys.map((k) => k.key)).not.toContain(`${MEDIA_PREFIX}/_incoming/${asset.id}`)
      })
      ...
      it('refuses a file whose bytes are not an image, whatever it claimed', async () => {
        ...
        await expect(service.completeUpload(actor, { mediaId: reservation.mediaId })).rejects.toThrow(
          CmsValidationError,
        )
        // Still pending, so the sweep collects it. Never half-ready.
        expect((await store.findById(reservation.mediaId))?.status).toBe('pending')
      })
    ...
    describe('usage', () => {
      ...
      async function page(slug: string, body: string, metadata: unknown = {}) {
        ...
        const [revision] = await db
          ...
          .returning()
        await db.update(schema.cmsPages).set({ wipRevisionId: revision.id }).where(eq(schema.cmsPages.id, row.id))
        return revision
      }
      ...
      it('derives usage from a body and blocks the trash while it stands', async () => {
        ...
        await expect(service.trash(actor, { id: asset.id })).rejects.toThrow(CmsMediaInUseError)
      })
      ...
      it('rebuilds to exactly what the incremental writer would have produced', async () => {
        ...
        created.add(asset.id)
        const row = await page('rebuild', `![A](${asset.permalink})\n\n![B](${asset.permalink})`)
    ...
    describe('retained versions pin their media', () => {
      ...
      it('refuses the trash while a published version still references it', async () => {
        ...
        const usage = await store.usageOf(asset.id)
        expect(usage.map((reference) => reference.revisionId)).toEqual([publication.id])
      ...
      it('releases it the moment the last retaining version is pruned', async () => {
        ...
        created.add(asset.id)
        const { pageRow, publication } = await publishedThenDropped(asset.permalink)
        await reconcileMediaUsage(db)
        expect(await store.isReferenced(asset.id)).toBe(true)
        ...
        await db
          ...
          .where(eq(schema.cmsPages.id, pageRow.id))
        await db.delete(schema.cmsPageRevisions).where(eq(schema.cmsPageRevisions.id, publication.id))

        expect(await store.isReferenced(asset.id)).toBe(false)
        await expect(service.trash(actor, { id: asset.id })).resolves.toMatchObject({ status: 'trashed' })
      })
      ...
      it('counts one page even when two of its versions use the image', async () => {
        ...
        expect((await store.usageOf(asset.id)).map((r) => r.pageId)).toEqual([pageRow.id, pageRow.id])
      })
    })
    ...
    describe('collections', () => {
      it('groups media without touching the object key', async () => {
        ...
        const asset = await upload(`${TEST_PREFIX}filed.png`, undefined, collection.id)
        created.add(asset.id)
      ...
      it('counts the media in each collection', async () => {
        ...
        for (const name of ['one', 'two']) {
          const asset = await upload(`${TEST_PREFIX}count-${name}.png`, undefined, collection.id)
          created.add(asset.id)
        }
      ...
      it('does not delete media when a collection is deleted', async () => {
        ...
        const asset = await upload(`${TEST_PREFIX}orphan.png`, undefined, collection.id)
        created.add(asset.id)
