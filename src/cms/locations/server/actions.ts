...
function failure(error: unknown): LocationActionResult<never> {
  if (error instanceof CmsLocationConflictError)
    return {
      ok: false,
      kind: "conflict",
  ...
  if (error instanceof CmsLocationSlugTakenError)
    return { ok: false, kind: "slug_taken", message: error.message };
  if (error instanceof CmsLocationInUseError)
    return {
      ok: false,
      kind: "in_use",
      message: error.message,
...
export async function renameLocationAction(input: {
  id: string;
...
export async function retireLocationAction(input: {
  id: string;
  expectedLockVersion: number;
