...
type View =
  | { kind: "list" }
  | { kind: "create" }
  | { kind: "edit" | "rename" | "retire"; location: ContentLocationWithUsage };

export function LocationManager({
  initialLocations,
}: {
  initialLocations: ContentLocationWithUsage[];
}) {
    ...
    <>
      ...
      >
        ...
        <span className="mt-2 block font-mono text-[13px] leading-[1.6] text-muted">
          Administra los lugares que conectan contenido de todas las secciones.
        </span>
      </button>
      {open && (
        <CmsModal
        ...
        >
          {view.kind === "list" ? (
            <LocationList
              ...
              onOpen={(kind, location) => {
                setError(null);
                setView({ kind, location });
              }}
              onClose={close}
            />
          ) : view.kind === "create" ? (
            <LocationForm
              ...
              onSubmit={(values) =>
                run(() => createLocationAction(values), "Ubicación creada.")
              }
            />
          ) : view.kind === "edit" ? (
            <LocationForm
              ...
              onSubmit={(values) =>
                run(
                  () =>
                    updateLocationAction({
                      id: view.location.id,
                      expectedLockVersion: view.location.lockVersion,
                      patch: values,
                    }),
...
function LocationList({
  ...
  locations: ContentLocationWithUsage[];
  notice: string | null;
  onCreate: () => void;
  onOpen: (
    kind: "edit" | "rename" | "retire",
      ...
      {locations.length === 0 ? (
        ...
        <ul className="mt-6 mb-0 list-none border-t border-line p-0">
          {locations.map((location) => (
            <li key={location.id} className="border-b border-line py-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-2">
                  <RowButton onClick={() => onOpen("edit", location)}>
                    Editar
                  </RowButton>
                  <RowButton onClick={() => onOpen("rename", location)}>
                    Dirección
                  </RowButton>
                  <RowButton danger onClick={() => onOpen("retire", location)}>
                    Eliminar
                  </RowButton>
                </div>
              </div>
...
function LocationForm({
  ...
  return (
    ...
    >
      ...
      <Actions
        busy={busy}
        submit={mode === "create" ? "Crear" : "Guardar"}
        onCancel={onCancel}
      />
    </form>
  );
...
function RenameForm({
  ...
  return (
    ...
    >
      ...
      <FormField label="Nueva dirección" help={`/ubicacion/${slug || "…"}`}>
        <input
          required
          value={slug}
          onChange={(event) => setSlug(slugifyLocation(event.target.value))}
          className={INPUT}
        />
      </FormField>
      <ActionError error={error} />
      <div className="mt-6 flex gap-2">
        <button
          type="submit"
          disabled={busy || slug === location.slug}
          className={PRIMARY}
        >
    ...
    <div className="mt-5">
      ...
      <div className="mt-6 flex gap-2">
        <button
          type="button"
          onClick={onConfirm}
          disabled={busy || location.usageCount > 0}
          className={DANGER}
        >
          {busy ? "Eliminando…" : "Eliminar ubicación"}
        </button>
        <DialogCancel onClick={onCancel} disabled={busy} />
      </div>
    </div>
...
const CARD =
  "block w-full cursor-pointer border border-line bg-card px-5 py-5 text-left text-ink transition-colors hover:border-accent focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent";
const INPUT =
...
const DANGER =
  "cursor-pointer border border-[var(--vendor-ochre)] px-3 py-2 font-mono text-micro tracking-label-wide text-[var(--vendor-ochre)] uppercase hover:bg-[var(--vendor-ochre)] hover:text-paper disabled:opacity-45";
