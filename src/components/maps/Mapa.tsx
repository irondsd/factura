...
// that carries the same numbers as text. It knows about none of the geographies
// it draws and nothing about what is being measured — the geometry and the
// wording both arrive as props.
//
// Wrappers bind it to a territory: `MapaCaba` for the 48 barrios and 15
// comunas, `MapaPba` for the 26 partidos of the Gran Buenos Aires and the 135
// of the province. Pages use those, never this.
//
...
//
// ── Why the geometry comes through props here but not from a page ──────────
// The path data is imported by the wrapper, which is itself a client module,
// so it lands in a JS chunk cached across the section. Passing it from a
// *server* component would be the mistake the wrappers exist to prevent: it
// would put 40 KB of paths in the page's HTML *and* again in the RSC payload.
//
...
// and dozens of polygons that on a phone are smaller than a fingertip. The
// table is the answer to all three, and it is why the map can stay simple.
//
...
export type MapView = {
  /** Which of the wrapper's path sets to draw. The wrapper decides what the
   * legal values are — "barrios" and "comunas" for CABA, "partidos" for PBA. */
  geo: string;
  regions: MapRegion[];
...
export function Mapa({
  ...
  /** Path sets by geo id — `paths[view.geo][region.id]` is an SVG `d`. */
  paths: Record<string, Record<string, string>>;
  /** The box every path set is drawn in. One per wrapper, shared by its geos,
   * which is what lets a switch swap them without the territory moving. */
  viewBox: string;
  /** An inert silhouette drawn under the regions — CABA on the GBA map, which
   * is the hole in the middle of the conurbano and is not part of the province
   * this page is about. Not hoverable and not in the table. */
  backdrop?: { d: string; label: string };
  title: string;
    ...
    <div>
      ...
      >
        ...
        >
          ...
          {/* Before the regions, so a shared border is painted over by the
              region that owns it rather than by the backdrop. `<title>` is what
              a screen reader gets for the one shape on the map that has no row
              in the table below. */}
          {backdrop && (
            <path
              d={backdrop.d}
              fill="var(--choro-backdrop)"
              stroke="none"
              pointerEvents="none"
            >
              <title>{backdrop.label}</title>
            </path>
          )}
