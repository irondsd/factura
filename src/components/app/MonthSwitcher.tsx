 ...
 * ledger actually has that month. */
function yearRows(selectable: string[]) {
  const have = new Set(selectable);
  const years = [...new Set(selectable.map((m) => m.slice(0, 4)))]
    .sort()
    .reverse();
  return years.map((year) => ({
    ...
    cells: MONTH_KEYS.map((key, i) => {
      ...
      return { key, month, enabled: have.has(month) };
    }),
  }));
}
...
export function MonthSwitcher({
  ...
  month: string;
  /** Every month the ledger can show, oldest → newest. */
  selectable: string[];
  ...
  const rows = yearRows(selectable);
        ...
        <>
          ...
          >
            ...
            <div
              ...
              className="max-h-[min(calc(2.5*var(--year-row)),50vh)] overflow-y-auto overscroll-contain sm:max-h-[calc(5*var(--year-row-sm))]"
            >
              {rows.map((row) => (
                <div
                  key={row.year}
