 ...
 *
 * The bill prints the total and the extraordinaria but not the ordinaria, so we
 * derive it: ordinaria = total − extraordinaria (rounded to the cent to avoid
 * float noise), falling back to the total when no extraordinaria was charged.
 */
export const mdaExpensasConfig: ParserConfig = {
  ...
  captures: [
    ...
    },
    {
      ...
      pattern: '(?:\\b\\d{3}\\s+|Consorcista\\s+)(?<num>\\d{1,3})\\s*(?:[°º]\\s*|-)(?<letter>[A-Z])\\b',
      outputs: {
  ...
  compute: [
    ...
    // Ordinaria = total − extraordinaria, rounded to the cent. When no
    // extraordinaria was charged, `extraordinary` is absent so the subtraction
    // yields undefined and we fall back to the whole total.
    { name: 'ordinaryRawCents', expr: '(amount - extraordinary) * 100' },
    { name: 'ordinaryCents', round: 'ordinaryRawCents' },
    { name: 'ordinaryNet', expr: 'ordinaryCents / 100' },
    { name: 'ordinary', coalesce: ['ordinaryNet', 'amount'] },
  ],
  ...
  custom: [
    { name: 'ordinary', source: 'ordinary', type: 'money' },
    { name: 'extraordinary', source: 'extraordinary', type: 'money' },
  ],
}
